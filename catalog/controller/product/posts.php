<?php
class ControllerProductPosts extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('product/posts');
		...
		$product_info = $this->model_catalog_posts->getProduct($this->request->get['product_id']);
	

		if ($product_info) {
			$url = '';
            
			$this->data['cms_type'] = $product_info['type'];
			$this->data['cms_title'] = $product_info['title'];
			$this->data['cms_content'] = $product_info['content'];
			$this->data['cms_id'] = $product_info['id'];
			
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/posts.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/product/posts.tpl';
			} else {
				$this->template = 'default/template/product/posts.tpl';
			}
