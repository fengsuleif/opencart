...
class ControllerAccountAddress extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')){
			$file = $_FILES;
				if(!$file["sfza"]["name"]){
					$this->error['sfza_4']=$this->language->get('error_sfza_4');
				}
				if(!$file["sfzb"]["name"]){
					$this->error['sfzb_4']=$this->language->get('error_sfza_4');
				}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$destination_folder="image/sfz/"; //上传文件路径
			$file = $_FILES;
			//print_r($file);
			if($file["sfza"]["name"]){
				...
				//	print_r($type);
				$type=pathinfo($file["sfza"]["name"])["extension"];
			
				$news_sfza_name=(int)$this->customer->getId()."_sfz_a_".time().".".$type;
			
				if(!move_uploaded_file ($file["sfza"]["tmp_name"], $destination_folder.$news_sfza_name))
				{
					$this->error['sfza_3']=$this->language->get('error_sfza_3');
				}
				...
				//
				$this->request->post['sfza']=$news_sfza_name;
			...
			if($file["sfzb"]["name"]){
					...
					$type=pathinfo($file["sfzb"]["name"])["extension"];
					$news_sfzb_name=(int)$this->customer->getId()."_sfz_b_".time().".".$type;
			
					if(!move_uploaded_file ($file["sfzb"]["tmp_name"], $destination_folder.$news_sfzb_name))
					{
					$this->error['sfzb_3']=$this->language->get('error_sfzb_3');
			}
			$this->request->post['sfzb']=$news_sfzb_name;
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//	print_r($type);
				...
				if(file_exists($destination_folder.$address_info['sfza'])){
					unlink($destination_folder.$address_info['sfza']);
				}
...
// 		//	print_r($file);
	...
	protected function getForm() {
		...
		if (isset($this->error['sfza_1'])) {
			$this->data['error_sfza_1'] = $this->error['sfza_1'];
		} else {
			$this->data['error_sfza_1'] = '';
		}
		
		if (isset($this->error['sfza_2'])) {
			$this->data['error_sfza_2'] = $this->error['sfza_2'];
		} else {
			$this->data['error_sfza_2'] = '';
		}
		if (isset($this->error['sfza_3'])) {
			$this->data['error_sfza_3'] = $this->error['sfza_3'];
		} else {
			$this->data['error_sfza_3'] = '';
		}
		if (isset($this->error['sfza_4'])) {
			$this->data['error_sfza_4'] = $this->error['sfza_4'];
		} else {
			$this->data['error_sfza_4'] = '';
		}
		if (isset($this->error['sfzb_1'])) {
			$this->data['error_sfzb_1'] = $this->error['sfzb_1'];
		} else {
			$this->data['error_sfzb_1'] = '';
		}
		
		if (isset($this->error['sfzb_2'])) {
			$this->data['error_sfzb_2'] = $this->error['sfzb_2'];
		} else {
			$this->data['error_sfzb_2'] = '';
		}
		if (isset($this->error['sfzb_3'])) {
			$this->data['error_sfzb_3'] = $this->error['sfzb_3'];
		} else {
			$this->data['error_sfzb_3'] = '';
		}
		if (isset($this->error['sfzb_4'])) {
			$this->data['error_sfzb_4'] = $this->error['sfzb_4'];
		} else {
			$this->data['error_sfzb_4'] = '';
		}
		...
		if (isset($this->request->post['sfza'])) {
			$this->data['sfza'] = $this->request->post['sfza'];
		} elseif (!empty($address_info)) {
			$this->data['sfza'] = $address_info['sfza'];
		} else {
			$this->data['sfza'] = '';
		}
		
		
		if (isset($this->request->post['sfzb'])) {
			$this->data['sfzb'] = $this->request->post['sfzb'];
		} elseif (!empty($address_info)) {
			$this->data['sfzb'] = $address_info['sfzb'];
		} else {
			$this->data['sfzb'] = '';
		}
	//	print_r($this->data);
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['address_1']) < 2) || (utf8_strlen($this->request->post['address_1']) > 128)) {
		...
		if($file["sfza"]["name"]){
			...
			if(!in_array($file["sfza"]["type"], $uptypes))
					
			{
				$this->error['sfza_1']= $this->language->get('error_sfza_1');
			}
		
			if($max_file_size < $file["sfza"]["size"])
		
			{
				$this->error['sfza_2']=$this->language->get('error_sfza_2');
			}
		...
		if($file["sfzb"]["name"]){
			...
			if(!in_array($file["sfzb"]["type"], $uptypes))
		
			{
				$this->error['sfzb_1']=$this->language->get('error_sfzb_1');
			}
		
			if($max_file_size < $file["sfzb"]["size"])
		
			{
				$this->error['sfzb_2']=$this->language->get('error_sfzb_2');
			}
