...
$tuijia_list=$this->config->get('featured_product');


	
$query = $this->db->query("SELECT pd.name, p.price , p.image ,p.product_id  FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id in  (" . $tuijia_list . ") AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
              
	foreach($query->rows as $product){
		
		 
		$template->data['tuijian_product_list'][] = array(
					'name'     => $product['name'],
					'image'     => $product['image'],
					'price'     => $product['price'],
			...
			$template->data['username'] =$replace['firstname'] ."  ".$replace['lastname'] ;
			$template->data['shipping_dizhi'] =$replace['country'] ."  ".$replace['zone'] ."  ".$replace['city'] ."  ".$replace['address_1'] ;
$template->data['postcode'] =$replace['postcode'];
