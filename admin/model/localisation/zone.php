...
class ModelLocalisationZone extends Model {
	...
	public function getZones($data = array()) {
		$sql = "SELECT *, z.name, z.status,c.name AS country FROM " . DB_PREFIX . "zone z LEFT JOIN " . DB_PREFIX . "country c ON (z.country_id = c.country_id) where 1=1";
		...
		if (!empty($data['filter_country_name'])) {
			$sql .= " AND c.name LIKE '%" . $this->db->escape($data['filter_country_name']) . "%'";
		}
